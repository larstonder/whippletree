...
func parseRunArgs(args []string) (event, targetName string, err error) {
	...
	for i := 0; i < len(rest); i++ {
		...
		// Strict, matching whippletree's own parsers: an unrecognized
		// flag is an error rather than being silently accepted as the
		// event name. The command line here is generated by
		// compile.Build, so anything else means a mismatch worth
		// surfacing instead of dispatching the wrong event.
		if strings.HasPrefix(rest[i], "-") {
			return "", "", fmt.Errorf("whippletree-hook: unknown flag %q", rest[i])
		}
		if event == "" {
			event = rest[i]
		}
