...
// TestInstallAcceptsOlderGeneratedByWording: overwrite protection keys
// on a stable prefix, not the exact line, so a shim written by an
// earlier whippletree is still recognised as ours. Matching the whole
// line would make any future rewording refuse to overwrite whippletree's
// own output.
func TestInstallAcceptsOlderGeneratedByWording(t *testing.T) {
	older := "// Generated by whippletree. Do not hand-edit; regenerate with `whippletree build`."
	if older == compile.GeneratedByLine {
		t.Fatal("fixture must differ from the current line to test anything")
	}
	...
	for _, tc := range []struct {
		...
		{"an older wording", older, true},
		{"the current line", compile.GeneratedByLine, true},
		{"hand-authored", "// my own plugin, do not touch", false},
		{"empty file", "", false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			dir := t.TempDir()
			dest := filepath.Join(dir, "whippletree.ts")
			if err := os.WriteFile(dest, []byte(tc.firstLine+"\nconst HOOK = \"x\"\n"), 0o644); err != nil {
				t.Fatal(err)
			}
			err := checkOverwriteAllowed(dest)
			if tc.wantOK && err != nil {
				t.Errorf("checkOverwriteAllowed = %v, want nil", err)
			}
			if !tc.wantOK && err == nil {
				t.Error("checkOverwriteAllowed = nil, want a refusal")
			}
		})
	}
}
