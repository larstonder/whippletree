...
| 2026-08-21 | copilot | 1.0.80 | Bring-up baseline, cross-reference only. Hooks fire end to end through the marketplace install route against this version, and the T3 fallback skill is discovered. `Stop` is mapped but ignores exit codes, verified against a `PreToolUse` control that does deny; it does block on a stdout JSON decision, which the dispatcher does not yet emit (issue #19). See `docs/copilot-probe-findings.md`. | (bring-up, not counted) | (this commit) |
